...
class laporan_rb_fokus extends CI_Controller
{
    ...
    public function export($formname, $id_laporan)
    {
        ...
        //for buat data
        $counter = 0;
        foreach ($this->data['fetch']['rbf'] as $rbf) {
            $rowspan_rincian = 0;

            foreach ($this->data['fetch']['rbfs'][$rbf['id_rb_fokus']] as $rbfs) {
                $rowspan_rincian += sizeof($this->data['fetch']['rbfk'][$rbfs['id_rb_fokus_sasaran']]);
            }
            $rowspan_rincian -= 1;
            ...
            $sheet->setCellValue('B' . $numrow, $rbf['rincian'])->mergeCells('B' . $numrow . ':B' . ($numrow + $rowspan_rincian));
            foreach ($this->data['fetch']['rbfs'][$rbf['id_rb_fokus']] as $rbfs) {
                $rowspan_program = sizeof($this->data['fetch']['rbfk'][$rbfs['id_rb_fokus_sasaran']]) - 1;
                if ($rowspan_program < 0) {
                    $rowspan_program = 0;
                }

                $sheet->setCellValue('C' . $numrow, $rbfs['sasaran'])->mergeCells('C' . $numrow . ':C' . ($numrow + $rowspan_program));
                $sheet->setCellValue('D' . $numrow, $rbfs['nama_program'])->mergeCells('D' . $numrow . ':D' . ($numrow + $rowspan_program));

                $flag2 = FALSE;
                $rbapk = reset($this->data['fetch']['rbfk'][$rbfs['id_rb_fokus_sasaran']]);
                ...
                foreach ($this->data['fetch']['rbfk'][$rbfs['id_rb_fokus_sasaran']] as $rbfk) {
                    if ($flag2) {
                        $sheet->setCellValue('E' . $numrow, $rbapk['nama_kegiatan']);
                        $sheet->setCellValue('F' . $numrow, $rbapk['indikator']);
                        $sheet->setCellValue('G' . $numrow, $rbapk['target_output']);
